...
- **Réponse directe à VM-60** (« Table d'événements aléatoires Markov »). La matrice EST le moteur cherché.
- **Piste pour VM-1 (Tension)** — la matrice est un candidat naturel pour mécaniser la Tension : *cadence d'événements* = pression d'exploration. Voir [[concept-tension]].
- **2d6 côté MJ ne contredit pas le D20 roll-under joueur** — c'est une table de prep, pas un test joueur.
...
- **Frictions de lexique** : DEX, CON, CHA, Perception, Moral, +1 attaque, 1d4 dégâts, avantage/désavantage joueur — tout doit être traduit en lexique Vermis (VIG/FIN/ESP, spectre T-4/T+4, dégâts END/attribut direct).
- **État *Fatigue*** : impact mécanique précis à arbitrer (perte d'END vs dégâts directs sur attribut).
...
- Quand on attaque **VM-1** (définir la Tension) — la matrice est un candidat fort de mécanisation.
- Quand on attaque **VM-60** (table d'événements Markov) — c'est directement la réponse.
