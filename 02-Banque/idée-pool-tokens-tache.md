...
**ADOPTÉE** comme nouveau Test de Tâche canonique le 2026-04-27. Remplace l'ancien système ampleur/dé-tracker/lecture directe (§4.4). Voir `01-Règles/Vermis_Rules.md` §4.4 pour la version officielle.
...
- **K dés/tour** au choix du joueur, plafonné par : tokens restants, points d'usure de l'outil, K=3 max sans outil, plafond contextuel MJ.
- **K=1** : aucun risque, aucun coût. La voie patiente reste patiente.
- **Doubles** = 1 conséquence (d6). **Triples** = 3 conséquences.
...
Pool de d6 (ampleur), seuil 4 par défaut, K dicté par qualité d'outil (2–5). Doubles = test de Réaction automatique. Expertise → modifie seuil.
...
- Test de Réaction systématique sur double → réplique le défaut « unifier dans la grammaire outil + dés efface les trois textures ».
- Roll-over en système majoritairement roll-under.

### V2 — Pool de dés d'outil avec usure-comme-cadence (2026-04-26, écartée)

Pool = ampleur en dés du type de l'outil. Complexité = seuil à dépasser. K plafonné par usure restante. Double = événement négatif annulable par test d'Action.
...
- Test d'Action systématique sur double → multiplication des mini-jets.
...
- **Conséquences tabulées (d6)** au lieu de test d'Action systématique : import élégant de la grammaire des doubles arcaniques, sans multiplier les jets.
